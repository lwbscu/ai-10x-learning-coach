...
选择后，所有课件内容、文件夹命名、测验题目、导航提示都会用你选择的语言输出。**但技术术语（如 MDP、Q-Learning、Bellman 方程）保留英文。**
...
learning/<主题-slug>/
├── _meta/
│   ├── profile.md          # 学习者档案（背景、目标、语言选择）
│   ├── progress.md         # 进度追踪（当前课、薄弱点、下一步）
│   └── mistakes.md         # 错题记录（日期、概念、错误理解、正确版本）
├── lesson-01-全局地图/
│   ├── notes.md            # 全局地图笔记
│   └── quiz.html           # 独立交互式测验
├── lesson-02-mdp详解/
│   └── index.html          # 交互式课件 + 嵌入式测验
├── lesson-03-价值函数/
│   └── index.html
└── ...
```

中文模式下文件夹用中文简称，英文模式下用英文 slug。技术术语（mdp、ppo 等）在两种模式下均保留英文。
...
语言选择只影响：UI 文字、文件夹命名、测验题目文本。**两种模式下的技术术语（MDP、Q-Learning、policy gradient）都保留英文**，确保概念准确、与国际社区一致。
...
互动性。HTML 课件可以内嵌测验、折叠深读、多视角切换，学习者不只是"读"而是"用"。Markdown 只适合第 1 课的地图笔记。
